...
@section('content')
    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header">
          <div class="row">
            <div class="col-lg-4">
              <div class="card-title d-inline-block">{{ __('All Users') }}</div>
            </div>
                    ...
                    <tbody>
                      @foreach ($admins as $admin)
                        <tr>
                          ...
                          <td>
                            <img src="{{ asset('assets/admin/img/organizer-photo/' . $admin->photo) }}" alt="admin image" width="45">
                          </td>
                          <td>{{ $admin->username }}</td>
                          <td>{{ $admin->email }}</td>
                          <td><?php echo $userrole = \App\Models\RolePermission::where('id',$admin->role_id)->first('name')->name; ?></td>
                          <td>
                            <form id="statusForm-{{ $admin->id }}" class="d-inline-block" action="{{ route('organizer.organizer_management.user.update_status', ['id' => $admin->id]) }}" method="post">
                              @csrf
  ...
  {{-- create modal --}}
  @include('organizer.site-user.create')

  {{-- edit modal --}}
  @include('organizer.site-user.edit')
@endsection
