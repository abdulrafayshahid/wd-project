    ...
    <div class="sidebar-wrapper scrollbar scrollbar-inner">
        <div class="sidebar-content">
            ...
            <ul class="nav nav-primary">
                ...
                <?php
                $Permissions = DB::table('role_permissions as role_permission')
                ->join('organizers as organizer', 'role_permission.id', 'organizer.role_id')
                ->where('organizer.id', auth()->user()->id)->first();
                
                $orgcheck = \App\Models\Organizer::where('id', auth()->user()->id)->first();
                if (isset($Permissions->permissions)) {
                    $rolePermissions = json_decode($Permissions->permissions);
        
                }
                ?>
                {{-- dashboard --}}
                @if (!empty($rolePermissions) && in_array('Organizer Dashboard', $rolePermissions) || empty($orgcheck->created_by))
                ...
                @endif
                @if (!empty($rolePermissions) && in_array('Event Management', $rolePermissions) || empty($orgcheck->created_by))
                    <li
                        class="nav-item 
          ...
          @elseif (request()->routeIs('organizer.event.ticket')) active
            ...
            @elseif (request()->routeIs('organizer.event.ticket')) show
              ...
              ">
                ...
                @endif
                @if (!empty($rolePermissions) && in_array('Event Bookings', $rolePermissions) || empty($orgcheck->created_by))
                    <li
                        class="nav-item
          ...
          @elseif (request()->routeIs('organizer.event_booking.report')) show @endif">
              ...
              @elseif (request()->routeIs('organizer.event_booking.details')) active @endif">
                ...
                @endif
                    @if (!empty($rolePermissions) && in_array('Withdraw', $rolePermissions) || empty($orgcheck->created_by))
                    <li
                        class="nav-item 
                    ...
                    @endif
                    @if ((!empty($rolePermissions) && in_array('Transaction', $rolePermissions)) || empty($orgcheck->created_by))
                    ...
                    @endif
                    @if (!empty($rolePermissions) && in_array('Pwa Scanner', $rolePermissions) || empty($orgcheck->created_by))
                    <li class="nav-item">
                    ...
                    @if (!empty($rolePermissions) && in_array('Support Ticket', $rolePermissions) || empty($orgcheck->created_by))
                    @endif
            ...
            @elseif (request()->routeIs('organizer.support_ticket.create')) active @endif">
              ...
              @elseif (request()->routeIs('organizer.support_tickets.message')) active @endif">
                                    ...
                                    <li
                                        class="{{ request()->routeIs('organizer.support_ticket.create') ? 'active' : '' }}">
                    ...
                    @endif
                    <li
                    class="nav-item @if (request()->routeIs('organizer.manual.ticket')) active
    @elseif (request()->routeIs('organizer.support_tickets.message')) active
    @elseif (request()->routeIs('organizer.manual.ticket.create')) active @endif">
                    <a data-toggle="collapse" href="#support_ticket2">
                        <i class="la flaticon-web-1"></i>
                        <p>{{ __('Manual Tickets') }}</p>
                        <span class="caret"></span>
                    </a>

                    <div id="support_ticket2"
                        class="collapse
      @if (request()->routeIs('organizer.manual.ticket')) show
      @elseif (request()->routeIs('organizer.support_tickets.message')) show
      @elseif (request()->routeIs('organizer.manual.ticket.create')) show @endif">
                        <ul class="nav nav-collapse">

                            <li
                                class="@if (request()->routeIs('organizer.manual.ticket')) active
      @elseif (request()->routeIs('organizer.support_tickets.message')) active @endif">
                                <a href="{{ route('organizer.manual.ticket') }}">
                                    <span class="sub-item">{{ __('All Manual Tickets') }}</span>
                                </a>
                            </li>
                            <li
                                class="{{ request()->routeIs('organizer.manual.ticket.create') ? 'active' : '' }}">
                                <a href="{{ route('organizer.manual.ticket.create') }}">
                                    <span class="sub-item">{{ __('Add Manual Ticket') }}</span>
                                </a>
                            </li>
                        </ul>
                    </div>
                  ...
                  @if (request()->routeIs('organizer.edit.profile')) active @endif">
                    ...
                    @if(empty($orgcheck->created_by))
                
                    <li class="nav-item">
                        <a href="{{ route('organizer.organizer_management.role_permissions') }}">
                            <i class="fal fa-key"></i>
                            <p>{{ __('Role & Permissions') }}</p>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ route('organizer.organizer_management.registered_user') }}">
                            <i class="fal fa-key"></i>
                            <p>{{ __('Users') }}</p>
                        </a>
                    </li>
